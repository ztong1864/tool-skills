...
class RelationService:
    ...
    def _derive_relations(self, relation: dict[str, Any]) -> list[dict[str, Any]]:
        created: list[dict[str, Any]] = []
        if relation["relation_type"] == "refersToVariant":
            step_ref = self._find_entity_by_id(relation["from"])
            if step_ref and step_ref.get("entity_type") == "WorkflowStepRef":
                workflow_id = step_ref.get("workflow_id")
                if workflow_id:
                    derived = {
                        "id": relation_id(workflow_id, "hasStep", relation["from"]),
                        "relation_type": "hasStep",
                        "from": workflow_id,
                        "to": relation["from"],
                        "derived_from": relation["id"],
                    }
                    if self._upsert_relation(derived, allow_existing=True):
