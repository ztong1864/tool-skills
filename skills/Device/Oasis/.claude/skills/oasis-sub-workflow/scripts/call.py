#!/usr/bin/env python3
"""
Oasis 绿洲实验平台 - 获取子工作流步骤参数
API: GET http://8.130.18.34:4002/api/workflow/sub-workflow/{subWorkflowId}/step-parameters
"""
...
SSH_PORT = 22
SSH_TIMEOUT = 10
BASE_URL = "http://8.130.18.34:4002/api/workflow/sub-workflow/{sub_workflow_id}/step-parameters"


def get_sub_workflow_params(sub_workflow_id, only_data=False, json_output=False, use_ssh=False,
    ...
    if only_data:
        ...
        steps = data["data"]
        print(f"[OK] 子工作流 {sub_workflow_id} 共 {len(steps)} 个步骤：\n")
        for step_id, step_list in steps.items():
            for s in step_list:
                print(f"  [{s.get('m')},{s.get('n')}] {s.get('name')}  (id: {step_id})")

    return data


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="获取绿洲子工作流步骤参数")
    parser.add_argument("--id", required=True, metavar="SUB_WORKFLOW_ID", help="子工作流 ID")
    parser.add_argument("--only-data", action="store_true", help="只输出 data 对象")
