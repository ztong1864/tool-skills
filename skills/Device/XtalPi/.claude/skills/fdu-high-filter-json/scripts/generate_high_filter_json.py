...
TARGET_DEFAULTS = {
    ...
    "unit_column": 0,
    "unit_row": 0,
    "unit_type": "exp_high_filtering_samples",
}
...
def validate_dst(dst_pos: str, resources: List[Dict[str, Any]]) -> None:
    for r in resources:
        ...
        if r.get("tray_QR_code") != DST_DEFAULT["tray_QR_code"]:
            continue
        return
    raise ValueError(f"高滤目标位不存在或不匹配：{dst_pos}")
...
def build_action(slot: Dict[str, Any], dst_pos: str, src_layout_code: str = None, resource_info: Dict[str, Any] = None) -> Dict[str, Any]:
    target = build_target(slot["source_layout_code"])
    # 从输入中获取unit_column和unit_row，如果不存在则使用默认值
    unit_column = slot.get("unit_column", target["unit_column"])
    unit_row = slot.get("unit_row", target["unit_row"])

    # 获取dilute_volume，如果不存在则使用0.0
    dilute_volume = slot.get("dilute_volume", 0.0)
    if dilute_volume > 0:
        dilute_volume = to_ml(dilute_volume, slot["unit"])

    # 从resource_info中获取substance
    substance = None
    if resource_info is not None:
        substance = resource_info.get("substance")

    # 获取src_layout_code
    if src_layout_code is None:
        if resource_info is not None and "layout_code" in resource_info:
            src_layout_code = resource_info["layout_code"]

    # 使用默认的resource_type和tray_QR_code
    ...
    return {
        ...
        "tray_QR_code": tray_QR_code,
        "unit_column": unit_column,
        "unit_row": unit_row,
        "unit_type": target["unit_type"],
        "process_json": {
            "src_layout_code": src_layout_code,
...
def save_output(action: Dict[str, Any]) -> Path:
    ...
    path = OUT_DIR / f"generated_high_filter_{ts}.json"
...
def main() -> int:
    ...
    # 获取resource_info
    resource_info = parsed.get("resource_info")

    action = build_action(slot, dst_pos, None, resource_info)
