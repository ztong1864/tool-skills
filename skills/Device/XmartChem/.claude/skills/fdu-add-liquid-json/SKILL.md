...
description: 这是化学实验流程的一部分，用于将液体加料 step_unit 转换为单步 exp_pipetting JSON；只处理单步液体加料映射，不负责整套协议编排或设备提交。
---
...
将液体加料 `step_unit` 转换为单步 `exp_pipetting` JSON。
...
- 输出：单个 `exp_pipetting` 动作 JSON 文件。
