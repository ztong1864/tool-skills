"""ToolUniverse-backed local tool bundle: 通用数据与文档工具集.

Bundles these local-compute ToolUniverse tools into ONE MCP server (see TOOL.md for
why one server, not one per tool): "DataQuality_assess", "get_tool_page_url".
...
_TOOL_NAMES = ["DataQuality_assess", "get_tool_page_url"]
_SLUG = "tooluniverse-utility-tools"
...
# without it, these 2 tools' @tool wrapper objects would be rebuilt from
